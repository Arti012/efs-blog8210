...
class CustomerController extends Controller
{
    ...
    public function show($id)
     {  
        ...
        ->Select(DB::raw('sum(purchase_price*shares) As total'))
        ->where('customer_id', $id)
        ->first();*/

        $total_acqvalue=Investment::where('customer_id', $id)->sum('acquired_value');
        $total_rectvalue=Investment::where('customer_id', $id)->sum('recent_value');

        $customer = Customer::findOrFail($id);
        return view('customers.show',compact('customer','stocks','investments','total_stkvalue','total_acqvalue','total_rectvalue'));
    }
